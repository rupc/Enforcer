...
def get_block(num):
    ...
    #      diff = abs(prev.number - block.number)
    #      if diff == 1:
    #          print_pretty_block(block)
    #          prev = block
    #          # print("Sending a new block")
    #          tx = build_transaction(block)
    #          send_transaction(specialTx)
    #          # print(data)
    #      elif diff > 1:
    #          print("blocks", diff, " delivered")
    #          blocks = []
    #          for i in range(prev.number + 1, prev.number + diff):
    #              blocks.append(web3.eth.getBlock(i))
    #          prev = block
    #      else:
    #          print("No new block")
    #      loop_count += 1
